...
- PDF 圧縮で、PDF 内の CMYK JPEG 画像を RGB へ正規化してから再圧縮するよう改善しました
- 埋め込み ICC プロファイルを持つ CMYK 画像では、その色情報を優先して変換します
- CMYK 画像を含む PDF 圧縮の回帰テストを追加し、非可逆圧縮の安定性を強化しました
...
- PDF 圧縮後に画像の色が崩れるのが不安
   - 1.0.3 では CMYK JPEG を RGB へ正規化してから圧縮するため、CMYK 画像を含む PDF でも従来より安定して処理できます
