...
- `flatten`: 注釈 / フォームを平坦化し、必要に応じて後処理圧縮を流す
...
- Model: `model/compress/compression_session.py`, `model/compress/compression_processor.py`, `model/compress/compression_dispatch.py`, `model/compress/native_compressor.py`, `model/compress/ghostscript_compressor.py`, `model/compress/settings.py`, `model/external_tools.py`
- View: `view/compress/*`

主なポイント:

- 入力は PDF、ZIP、フォルダ
- UI は Native / Ghostscript の 2 タブ構成で、`CompressionSession.engine` が選択中エンジンの正本になる
- Native タブの圧縮モードは `lossy`、`lossless`、`both`
- 既定値は `model/compress/settings.py` に集約される
- `model/external_tools.py` が外部バイナリ解決の正本で、Ghostscript は `winreg -> PATH -> bundled vendor binary`、pngquant は `PATH -> bundled vendor binary -> Pillow fallback` の順で扱う
- `model/compress/compression_dispatch.py` が engine ごとの入口をまとめ、native と Ghostscript の切り替えを担当する
- `model/compress/ghostscript_compressor.py` はプリセット / カスタム DPI からコマンドラインを組み立て、基底フラグと互換性 floor を適用し、必要に応じて後段 pikepdf 可逆最適化を流す
...
- PNG 量子化で `pngquant` を使うときは、解決済み実行ファイルパスを shell なしで起動し、専用一時ディレクトリ、タイムアウト、Windows の `CREATE_NO_WINDOW` を付けて実行する
...
- `FlattenSession` は入力一覧、進捗件数、warning 件数、後処理圧縮設定、出力命名規則、Windows path 制約判定を保持する
- `FlattenProcessor.prepare_batch()` は探索、非 PDF / missing input の skip、既存出力との conflict 分離、出力パス長の preflight failure 化を担当する
- `FlattenProcessor.start_flatten()` は worker thread を起動し、result queue へ success / warning / failure / skipped / progress / finished / cancelled を順次積む
- Presenter は `MainWindow.schedule()` を使って polling し、進捗文言、完了ダイアログ、cancel 後 close を制御する
- 実際の平坦化責務は `fitz.Document.bake()` に寄せ、annotation と widget を通常ページ内容へ焼き込んだ後、`Flatten -> Temp A -> Ghostscript -> Temp B -> publish` の順で後処理圧縮を差し込める
- flatten 後の Ghostscript は `model/compress/ghostscript_compressor.py` を再利用し、必要に応じて pikepdf 既定可逆最適化を追加する
- フラット化自体が成功し、後段圧縮だけが失敗した場合は Temp A の flattened PDF を publish し、warning として利用者へ通知する
- 失敗系として、invalid PDF、encrypted PDF、保存先 PermissionError、MAX_PATH 超過、broken appearance 系、Ghostscript 後処理失敗を failure / warning / skip 集計へ落とし、バッチ全体の継続性を優先する
...
Ghostscript 統合後の重点確認点:

- `tests/model/test_external_tools.py`: Ghostscript / pngquant の解決順と bundled fallback
- `tests/model/compress/test_ghostscript_compressor.py`: Ghostscript コマンド、基底フラグ、後段 pikepdf 連携
- `tests/model/compress/test_compression_dispatch.py`, `tests/model/compress/test_compression_processor.py`: engine dispatch と ZIP 経路
- `tests/model/flatten/test_flatten_processor.py`: flatten 後圧縮成功、失敗時 fallback publish、cancel cleanup
- `tests/presenter/test_flatten_presenter.py`, `tests/view/flatten/test_flatten_view.py`: 後処理トグル、部分成功メッセージ、Ghostscript 未検出 UI
